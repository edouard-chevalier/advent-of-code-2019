...
namespace Day5 {
  class Program {
    ...
    public class State {
      ...
      int NextValue( bool paramMode = false ) {
        int res = ints[pointer++];
        if ( paramMode ) {
          return res;
        }
      ...
      void Multiply( OpCode op) {
        int leftVal = NextValue(op.GetMode( 0 ));
        int rightVal = NextValue(op.GetMode( 1 ));
        WriteValue( leftVal * rightVal, NextValue( true ));
      }
      void Add( OpCode op ) {
        int leftVal = NextValue(op.GetMode( 0 ));
        int rightVal = NextValue(op.GetMode( 1 ));
      ...
      void Input( OpCode op ) {
        WriteValue( inputs.Pop(), NextValue(/*op.GetMode( 0 )*/ true ));
      }

      void Ouput( OpCode op ) {
        var value = NextValue( true );
        outputs.Add( value );
      }
    }
  }
