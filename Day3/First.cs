...
namespace Day3 {
  public class First {
    ...
    static void SomeMain( string[] args ) {
      Console.WriteLine( "Hello Worlgd!" );

      var cells = new Dictionary<CellPosition, Cell>();

      void UpdateCell( CellPosition cellPosition, bool isPath1 ) {
...
foreach ( var cell in cells.Values.Where( c=>c.IsIntersection ) ) {
  currentMin = Math.Min( currentMin, cell.ManhattanDistance() );
}
...
Console.Write( currentMin );
    ...
    public struct Cell {
      ...
      public bool IsIntersection => Path1 && Path2;
